<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit '.$toUpdate->className.' Class') }}
        </h2>
    </x-slot>
    <!-- Session Status -->
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 pt-4 w-1/3">
        <div class="overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 border-b border-gray-200">
                <form method="POST" action="{{ route('animalclass.update', $toUpdate->id) }}">
                    @csrf
                    @method('PUT')
                    <!-- className -->
                    <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:w-2/3 my-0 py-0">
                            <x-input id="className" class="block mt-1 w-full" type="text" name="className" value="{{ $toUpdate->className }}" required autofocus />
                            @error('className')
                    ...
                    <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:w-2/3">
                            <x-input id="section" class="block mt-1 w-full" type="text" name="section" value="{{ $toUpdate->section }}" required />
                            @error('section')
                                <p class="text-red-500 text-xs italic">{{$message}}</p>
                            @enderror
                        </div>
