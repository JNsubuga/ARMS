                        ...
                        @forelse ($staffmembers as $staffmember)
                            <tr class="bg-white border-b dark:bg-gray-100 dark:border-gray-200">
                                <td class="py-1 px-6 font-bold text-gray-900 whitespace-nowrap dark:text-black">
                                    <a href="{{ route('staffmember.show', $staffmember->id) }}">
                                        {{ $staffmember->Names }}
                                    </a>
                                </td>
                                <td class="py-1 px-6">
                                    {{ date('d/m/Y', strtotime($staffmember->DoB)) }}
                                </td>
                                <td class="py-1 px-6">
                                    {{ $staffmember->Address }}
                                </td>
                                <td class="py-1 px-6">
                                    {{ $staffmember->animalclass->section }}
                                </td>
                                <td class="flex py-1 px-6">
                                    <a href="{{route('staffmember.edit', $staffmember->id)}}" class="text-blue-600 italic pr-4">Edit</a>
                                    <form action="{{route('staffmember.destroy', $staffmember->id)}}" method="post" onsubmit="return confirm('Are you sure! You need to Delete this Record?!')">
                                        @csrf
                                        @method('DELETE')
