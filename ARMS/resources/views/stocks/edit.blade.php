    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 pt-4 w-1/3">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('stock.update', $toUpdate->id) }}">
                    @csrf
                    @method('PUT')
                    <!-- product_id -->
                    <div class="md:flex md:items-center mb-2">
                        ...
                        <div class="md:w-2/3 my-0 py-0">
                            <x-selectinput name="product_id" id="product_id">
                                <option value="{{ $defaultProduct->id }}" disabled selected hidden>{{$defaultProduct->Name}}</option>
                    ...
                    <div class="md:flex md:items-center mb-2">
                        ...
                        <div class="md:w-2/3 flex">
                            <div class="md:w-1/2 mr-1">
                                <x-input id="Quantity" class="block mt-1 w-full" type="text" name="Quantity" value="{{ $toUpdate->Quantity }}" />
                                @error('Quantity')
                                    <p class="text-red-500 text-xs italic">{{$message}}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:w-1/2 mt-1">
                                ...
                                <option value="{{ $defaultUnit->id }}" disabled selected hidden>{{$defaultUnit->Abbriviation}}</option>
