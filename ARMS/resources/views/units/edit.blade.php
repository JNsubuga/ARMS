    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 pt-4 w-1/3">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('unit.update', $toUpdate->id) }}">
                    @csrf
                    @method('PUT')
                    <!-- Unit -->
                    <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:w-2/3">
                            <x-input id="Unit" class="block mt-1 w-full" type="text" name="Unit" value="{{ $toUpdate->Unit }}" required autofocus />
                            @error('Unit')
                    ...
                    <div class="md:flex md:items-center mb-6">
                        ...
                        <div class="md:w-2/3">
                            <x-input id="Abbriviation" class="block mt-1 w-full" type="text" name="Abbriviation" value="{{ $toUpdate->Abbriviation }}" required />
                            @error('Abbriviation')
                                <p class="text-red-500 text-xs italic">{{$message}}</p>
                            @enderror
                        </div>
